...
class RestaurantOrdersDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ->escapeColumns([])
            ->addColumn('check', 'admin.orders.restaurant.datatables.check')
            ->editColumn('status.name', function ($q) {
                return $q->status->getTranslations('name')['ar'];
            })

            ->rawColumns(['action','status'])
            ->addColumn('actions', 'admin.orders.restaurant.datatables.actions');
     ...
     */
    public function query(RestaurantOrder $model)
    {
