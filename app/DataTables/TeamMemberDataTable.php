...
class TeamMemberDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ->escapeColumns([])
            ->addColumn('check', 'admin.team_members.datatables.check')
            ->editColumn('name', function ($q) {
                return "<a href='".route('teamMembers.show', $q->id)."'>".$q->getTranslations('name')['ar']."</a>";
            })

            ->rawColumns(['action','status'])
            ->addColumn('actions', 'admin.team_members.datatables.actions');
    }

    /**
     * Get query source of dataTable.
     *
     * @param \App\Models\TeamMember $model
    ...
    {
        return 'TeamMember_' . date('YmdHis');
    }
}
