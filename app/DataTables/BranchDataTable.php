...
class BranchDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $dataTable
            ->escapeColumns([])
            ->addColumn('check', 'admin.branches.datatables.check')
            ->editColumn('name', function ($q) {
                return "<a href='".route('branches.show', $q->id)."'>".$q->getTranslations('name')['ar']."</a>";
            })
            ->rawColumns(['action','status'])
            ->addColumn('actions', 'admin.branches.datatables.actions');
    }
